...
use App\Http\Services\Scrapers\ValdemarsroService;
...
class ScrapeController extends Controller
{
    ...
    public function index(Request $request, ScrapeService $scrapeService)
    {
        ...
        switch ($hostname) {
            ...
            case 'www.valdemarsro.dk':
                $valdermarsroService = app(ValdemarsroService::class, ['url' => $request->get('url')]);
                $content = $valdermarsroService->scrapeAndGetContent();

                $recipe = \App\Http\Adapters\Valdemarsro\RecipeAdapter::adapt($content);
                $recipe->save();
